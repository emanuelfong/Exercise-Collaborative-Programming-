...
class Cities:
    """Tracks cities and their populations; can tell a user tje closest cities to a given location
    Attributes:
    cities(list of dict): information about cities. Each item in the list is a 
    dict with a following keys:
    "name": 
        a string containing the name of the city in English
    ...
    def __init__(self,filepath):
        """Read city from a file and store it in the attibute class
        
        
        Args:
        filepath(str): path to a text file with name,latitude,longitude,and 
        population of cities,one set of values per line
        
        Side effects:
        Set the cities attributes"""
        self.cities = []
        with open(filepath,"r", encoding="utf-8") as f:
                for city_data in f:
                    name,lat,lon,pop= city_data.strip().split("\t") 
                    {"name":name,
    ...
    def nearest(self,lat,lon,min_population=0,n=10):
        """Define the n nearest end cities to the location ("lat","lon") with a 
        population that is no less than "min population"
        Args:
        lat(float): a latitude
        lon(float): a longitude
        min_population(int,optional): if specified, only return cities whose 
        population is greater than or equal to this value
        Default: 0
        n(int): a number of results to return. Default:0
    
        Returns:
        list of dict: dictionaries from "self.cities", sorted in increasing order 
        from the location specified by the user
        """
        """List comprehension syntax
        [EXPR for INTERVAR in ITERABLE if CONDITION]"""
        
        big_cities= [x for x in self.cities if x["pop"]] >= min_population
        
        big_cities.sort(key=lambda x:get_dist((lat), (lon), 
                                              (x["lat"], x.get("lon)"))))
